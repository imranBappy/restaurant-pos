...
import { UserFilterState } from "./staffs-data-table"
...
export function TableFilters({ filters, onFilterChange }: FiltersProps) {
  ...
  let roles: OPTION_TYPE[] = roles_data?.roles?.map((role: ROLE_TYPE) => ({
    value: role.id,
    label: role.name
  }))
  
  roles = roles?.filter((role: OPTION_TYPE) => role.label !== 'CUSTOMER');
