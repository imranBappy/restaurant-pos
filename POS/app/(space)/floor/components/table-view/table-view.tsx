...
const TableView = ({
    ...
    return (
        // <div className={cn("h-[calc(100vh-50px)]", sidebarContext?.open ? "w-[calc(100vw-300px)]" : "w-[calc(100vw-85px)]")}>
        <div className=" p-3">
            {/* Categories Section */}
            ...
            >
                <div className={cn("relative", sidebarContext?.open ? "w-[calc(100vw-389px)]" : "w-[calc(100vw-180px)]")}>
                    <div
                        className="overflow-x-auto mb-3 mt-1 ml-3 w-full scrollbar-hide"
                        id="categoryScroll"
                        style={{
                            overflowX: 'auto',
                ...
                <Button disabled={tableState?.length === 0} onClick={() => {
                    if (onClickBook) {
                        onClickBook()
                    }
                    handleNext()

                }} className="  w-28 ">Book {tableState.length || ""}</Button>
            </div>
            <div
            ...
            >
                {
                    loading && !floorTables_res ? <div className="w-full flex flex-wrap ">
                        <Loading />
                    </div> :
                            ...
                            >
                                {
                                    ...
                                    tables.map((item: FLOOR_TYPE) => (
                                        <TableCard
                                            handleBookUnbook={() => handleBookUnbook(item)}
                                            table={{
                                                id: item.id,
                                                name: item.name,
                                                status: item.isBooked ? "busy" : 'free',
                                                size: 10
                                            }}
                                            key={item.id}
                                            handleTableClick={() => handleAddToCart(item)}
                                            selected={
                                                !!tableState.find(tb => tb.id === item.id)
                                            }
                                        />
                                    ))
