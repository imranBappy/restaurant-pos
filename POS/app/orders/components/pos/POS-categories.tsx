...
const mapCategoryData = (edges: MAP_CATEGORY_TYPE[]): CATEGORY_TYPE[] => edges?.map((item: { node: CATEGORY_TYPE }) => ({
    ...
    subcategories: item.node.subcategories
})) || []
...
const POSCategories = ({ filters, handleFilterChange }: POSCategoriesProps) => {
    ...
    const { data: category_res, fetchMore: fetchMoreCategory } = useQuery(CATEGORIES_QUERY, {
        variables: {
            offset: 0,
    ...
    const categories = mapCategoryData(category_res?.categories?.edges)
    const sidebarContext = useContext(SidebarContext)


    const fetchMoreCategories = async () => {
        try {
            const currentLength = categories.length || 0;
            await fetchMoreCategory({
                ...
                updateQuery: (prev, { fetchMoreResult }) => {
                    ...
                    return {
                        categories: {
                            __typename: prev.categories.__typename,
                            totalCount: fetchMoreResult.categories.totalCount,
        ...
        <div className={cn("relative", sidebarContext?.open ? "w-[calc(100vw-754px)]" : "w-[calc(100vw-547px)]")}>
            ...
            >
                ...
                <InfiniteScroll
                    ...
                    hasMore={(category_res?.categories?.totalCount || 0) > (categories.length || 0)}
