...
export const OrderChannelTable = () => {
    ...
    const { loading, data: res, fetchMore } = useQuery(ORDER_CHANNELS_QUERY, {
        variables: {
    ...
    const payments: ORDER_CHANNEL_TYPE[] = res?.orderChannels?.edges?.map(({ node }: { node: ORDER_CHANNEL_TYPE }) => ({
        id: node.id,
        name: node.name,
        type: node.type,
